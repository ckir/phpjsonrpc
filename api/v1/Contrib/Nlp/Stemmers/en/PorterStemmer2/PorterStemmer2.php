<?php
/**
* https://github.com/schkovich/porter-stemmer/tree/master/Libs/PorterStemmer
*/
class PorterStemmer2 {
	...
	public function __construct() {
		...
		$this->exceptions = new \ArrayIterator ( array (
				"skis" => "ski",
				"skies" => "sky",
				"dying" => "die",
				"lying" => "lie",
		...
		$this->exceptions2 = new \ArrayIterator ( array (
				"inning",
				"outing",
				"canning",
				"herring",
		...
		$this->step1bReplacements = new \ArrayIterator ( array (
				"eedly" => "ee",
				"ingly" => "",
				"edly" => "",
				"eed" => "ee",
		...
		$this->step2Replacements = new \ArrayIterator ( array (
				"ization" => "ize",
				"iveness" => "ive",
				"fulness" => "ful",
				"ational" => "ate",
		...
		$this->step3Replacements = new \ArrayIterator ( array (
				"ational" => "ate",
				"tional" => "tion",
				"alize" => "al",
				"icate" => "ic",
		...
		$this->step4Replacements = new \ArrayIterator ( array (
				"ement",
				"ment",
				"able",
				"ible",
	...
	private function computeR1R2() {
		...
		if (($this->sb->count () >= 5) && ((0 === strcmp ( "gener", $this->sb->SubStr ( 0, 5 ) )) || (0 === strcmp ( "arsen", $this->sb->SubStr ( 0, 5 ) ))))
			$this->r1 = 5;
		
		if (($this->sb->count () >= 6) && (0 === strcmp ( "commun", $this->sb->SubStr ( 0, 6 ) )))
			$this->r1 = 6;
			/*
	...
	private function isShortSyllable($offset) {
		if ((0 === $offset) && ($this->isVowel ( 0 )) && (! $this->isVowel ( 1 ))) :
			return true;
		 elseif ((($offset > 0) && ($offset < ($this->sb->count () - 1))) && $this->isVowel ( $offset ) && ! $this->isVowel ( $offset + 1 ) && ($this->sb->offsetGet ( $offset + 1 ) != 'w' && $this->sb->offsetGet ( $offset + 1 ) != 'x' && $this->sb->offsetGet ( $offset + 1 ) != 'Y') && ! $this->isVowel ( $offset - 1 )) :
			return true;
		 else :
			return false;
		endif;
	...
	private function isShortWord() {
		if (($this->r1 >= $this->sb->count ()) && ($this->isShortSyllable ( $this->sb->count () - 2 )))
			return true;
		else
			return false;
	}
	...
	private function step0() {
		if (($this->sb->count () >= 3) && ("'s'" === $this->sb->SubStr ( $this->sb->count () - 3, 3 ))) :
			$this->sb->Remove ( $this->sb->count () - 3, 3 );
		 elseif (($this->sb->count () >= 2) && ("'s" === $this->sb->SubStr ( $this->sb->count () - 2, 2 ))) :
			$this->sb->Remove ( $this->sb->count () - 2, 2 );
		 elseif ('\'' === $this->sb->offsetGet ( $this->sb->count () - 1 )) :
			$this->sb->Remove ( $this->sb->count () - 1, 1 );
		endif;
		;
	}
	private function step1a() {
		if ((($this->sb->count () >= 4) && ("sses" === $this->sb->SubStr ( $this->sb->count () - 4, 4 )))) :
			$this->sb->Replace ( "sses", "ss", $this->sb->count () - 4, 4 );
		 elseif (($this->sb->count () >= 3) && (($this->sb->SubStr ( $this->sb->count () - 3, 3 ) == "ied") || ($this->sb->SubStr ( $this->sb->count () - 3, 3 ) == "ies"))) :
			$this->sb->Replace ( $this->sb->SubStr ( $this->sb->count () - 3, 3 ), ($this->sb->count () > 4) ? 'i' : 'ie', $this->sb->count () - 3, 3 );
		 elseif (($this->sb->count () >= 2) && (($this->sb->SubStr ( $this->sb->count () - 2, 2 ) == "us") || ($this->sb->SubStr ( $this->sb->count () - 2, 2 ) == "ss"))) :
			return;
		 elseif (($this->sb->count () > 0) && ($this->sb->SubStr ( $this->sb->count () - 1, 1 ) == "s")) :
	...
	private function step1c() {
		if (($this->sb->count () > 0) && (($this->sb->offsetGet ( $this->sb->count () - 1 ) == 'y' || $this->sb->offsetGet ( $this->sb->count () - 1 ) == 'Y')) && ($this->sb->count () > 2) && (! $this->isVowel ( $this->sb->count () - 2 )))
			$this->sb->offsetSet ( $this->sb->count () - 1, 'i' );
	}
	private function step2() {
		$this->step2Replacements->rewind ();
		while ( $this->step2Replacements->valid () ) :
			if (($this->sb->count () >= strlen ( $this->step2Replacements->key () )) && ($this->sb->SubStr ( $this->sb->count () - strlen ( $this->step2Replacements->key () ), strlen ( $this->step2Replacements->key () ) ) == $this->step2Replacements->key ())) :
				if (($this->sb->count () - strlen ( $this->step2Replacements->key () )) >= $this->r1) :
					switch ($this->step2Replacements->key ()) :
						case "ogi" :
							if (($this->sb->count () > 3) && ($this->sb->offsetGet ( $this->sb->count () - strlen ( $this->step2Replacements->key () ) - 1 ) == 'l'))
								$this->sb->Replace ( $this->step2Replacements->key (), $this->step2Replacements->current (), $this->sb->count () - strlen ( $this->step2Replacements->key () ), strlen ( $this->step2Replacements->key () ) );
							
							return;
						
						case "li" :
							if (($this->sb->count () > 1) && in_array ( $this->sb->SubStr ( $this->sb->count () - 3, 1 ), $this->validEndings, true ))
								$this->sb->Remove ( $this->sb->count () - 2, 2 );
	...
	private function step5() {
		if ($this->sb->count () > 0) :
			if (($this->sb->offsetGet ( $this->sb->count () - 1 ) == 'e') && ((($this->sb->count () - 1) >= $this->r2) || ((($this->sb->count () - 1) >= $this->r1) && (false === $this->isShortSyllable ( $this->sb->count () - 3 ))))) :
				$this->sb->Remove ( $this->sb->count () - 1, 1 );
			 elseif (($this->sb->offsetGet ( $this->sb->count () - 1 ) == 'l') && (($this->sb->count () - 1) >= $this->r2) && ($this->sb->offsetGet ( $this->sb->count () - 2 ) == 'l')) :
				$this->sb->Remove ( $this->sb->count () - 1, 1 );
			endif;
