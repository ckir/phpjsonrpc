...
namespace Local\Greek\Nlp\Tokenizers\WhitespaceTokenizer;
...
class WhitespaceTokenizer {
	...
	public function tokenize($content) {
		...
		for($i = 0; $i < count ( $tokens ); $i ++) {
			$token = trim ( $tokens [$i] );
			$token = \Local\Helpers\Helpers\Helpers::unquote ( $token, '""\'\'' );
			$first = mb_substr ( $token, 0, 1, 'UTF-8' );
			if (! \Local\Greek\Helpers\GrHelpers::gr_isalphanumeric ( $first )) {
				$token = mb_substr ( $token, 1, 'UTF-8' );
			}
			$last = mb_substr ( $token, - 1, 1, 'UTF-8' );
			if (! \Local\Greek\Helpers\GrHelpers::gr_isalphanumeric ( $last )) {
